...
type MoveToTS<T extends string> = T extends "u8" | "u16" | "u32" | "i8" | "i16" | "i32" ? number : T extends "u64" | "u128" | "u256" | "i64" | "i128" | "i256" ? bigint : T extends "bool" ? boolean : T extends "signer" | "&signer" ? SupraAccount : T extends "address" ? AccountAddressInput : T extends "vector<u8>" ? string | Uint8Array | ArrayBuffer | number[] : T extends `0x1::object::Object<${infer _U}>` ? AccountAddressInput : T extends "0x1::option::Option<u8>" ? string | number | null : T extends `0x1::option::Option<${infer U}>` ? MoveToTS<U> | null : T extends `vector<${infer U}>` ? MoveToTS<U>[] : T extends `0x1::string::String` ? string : T extends `${infer CA}::${infer MN}::${infer S}` ? `${CA}::${MN}::${S}` : unknown;
...
declare class Methods {
    ...
    */
     ...
     * @group Methods
     */
    viewRaw<T extends Array<unknown>>(args: InputViewRawFunctionData): Promise<T>;
}
...
declare class ExtendedRawTransaction extends TxnBuilderTypes.RawTransaction {
    private config;
    constructor(config: NetworkConfig, // pass whatever you need
...
declare function addAddressPadding(address: string): string;
...
export { Account, type AccountAddressInput, type AccountData, type AccountSignature, type Args, type Authenticator, type AutoTransactionResponse, type AutomationAuthenticator, type AutomationRecordData, type AutomationRecordTransactionResponse, type AutomationRegistrationParamV1JSON, type AutomationRegistrationParams, type AutomationRegistrationParamsV1, type AutomationRegistrationParamsV2, type AutomationRegistrationPayloadJSON, type AutomationRegistrationPayloadResponse, type AutomationTransactionResponse, Block, type BlockHeader, type BlockMetadataTransactionResponse, Build, Coin, type CoinInfo, type CommittedTransactionResponse, Contract, type ContractsFromABI, type ConvertGenerics, DEFAULT_CHAIN_ID, DEFAULT_ENABLE_SIMULATION, DEFAULT_GAS_PRICE, DEFAULT_MAX_GAS_FOR_SUPRA_TRANSFER_WHEN_RECEIVER_EXISTS, DEFAULT_MAX_GAS_FOR_SUPRA_TRANSFER_WHEN_RECEIVER_NOT_EXISTS, DEFAULT_MAX_GAS_UNITS, DEFAULT_RECORDS_ITEMS_COUNT, DEFAULT_REQUEST_TIMEOUT_MS, DEFAULT_RPC_VERSION, DEFAULT_TXN_TIMEOUT_SEC, DEFAULT_TX_EXPIRATION_DURATION, DEFAULT_WAIT_FOR_TX_COMPLETION, DELAY_BETWEEN_POOLING_REQUEST, type DeepReadonly, type DkgAuthenticator, type DkgTransactionOutput, type DkgTransactionPayload, type Ed25519Signature, type EmptyTransactionOutput, type EnableTransactionWaitAndSimulationArgs, type EntryArgs, type EntryFunctionJSON, type EntryFunctionPayload, type EntryFunctionPayloadJSON, type EntryFunctionPayloadResponse, type EntryFunctionsFromABI, type Event, type EventGuid, Events, Faucet, type FaucetTransactionResponse, type FeePayerSignature, type FinalizedBlockHeader, type FunctionTypeArgs, FungibleAsset, type FungibleAssetMetadata, type GasPrice, type Headers, type InputViewFunctionData, type InputViewRawFunctionData, MAX_RETRY_FOR_TRANSACTION_COMPLETION, MILLISECONDS_PER_SECOND, type MapArgs, type MapTypeArgs, Methods, type MoveAbility, type MoveEvent, type MoveFunction, type MoveFunctionGenericTypeParam, type MoveFunctionId, type MoveFunctionVisibility, type MoveInnerAuthenticator, type MoveModule, type MoveModuleBytecode, type MoveModuleId, type MoveModules, type MoveResource, type MoveScriptBytecode, type MoveStruct, type MoveStructField, type MoveStructId, type MoveToTS, type MoveTransactionAuthenticator, type MoveTransactionOutput, type MoveTransactionPayload, type MoveValue, MoveVmError, type MultiAgentSignature, type MultiEd25519Signature, type MultisigPayloadJSON, type MultisigPayloadResponse, type MultisigTransactionPayload, Network, type NetworkConfig, NetworkInfo, OBJECT_CORE, type OptionalTransactionArgs, type OptionalTransactionPayloadArgs, type OracleAuthenticator, type OracleTransactionOutput, type OracleTransactionPayload, type PaginatedResponse, type PendingTransactionResponse, RAW_TRANSACTION_SALT, RAW_TRANSACTION_WITH_DATA_SALT, type RawTxnJSON, type ReturnTypes, SUPRA_COIN_TYPE, SUPRA_FRAMEWORK_ADDRESS, type ScriptArgumentJson, type ScriptPayloadJSON, type ScriptPayloadResponse, type SendTxnPayload, type SimpleEntryFunctionArgumentTypes, type SimpleEntryFunctionArgumentTypesRaw, Simulate, type SingleSenderSignature, type StructFromABI, Submit, SupraAPIError, type SupraAPIErrorOptions, SupraClient, type SupraConfig, type SupraTransactionResponse, Table, type TableItemRequest, type TableKeyValueType, type TableKeyValueTypeHelper, Transaction, type TransactionOutput, type TransactionPayload, type TransactionPayloadJSON, type TransactionPayloadResponse, type TransactionQueryType, type TransactionResponse, TransactionStatus, TransactionType, type TypeArgument, type UserTransactionResponse, type ViewFunctionsFromABI, type WaitForTransactionOptions, addAddressPadding, getFunctionParts, isMoveVmError, standardizeAddress };
